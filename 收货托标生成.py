...
def barcode_png_base64(sku: str) -> str:
    bc = build_barcode_obj(sku)
...
def render_label_preview(container_no: str, client_code: str, sku: str):
    prefix, last4 = split_container(container_no)
    barcode_b64 = barcode_png_base64(sku)
    ...
    ">
        ...
        <div style="display:flex; justify-content:center; margin-top:8px;">
            <img src="data:image/png;base64,{barcode_b64}" style="max-width:82%; height:64px;" />
        </div>
...
if st.button("生成并下载 PDF", type="primary", disabled=all_records.empty):
    try:
        ...
        filename = "receiving_labels_batch.pdf"
        st.success("PDF 已生成。")
        st.download_button(
            label="下载 PDF",
            data=pdf_bytes,
